              ...
              <div className="flex gap-8">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap mb-3">
                    <span className="text-[#46d369] font-semibold text-sm">{matchScore}% {t('media.rating')}</span>
                ...
                <div className="w-44 flex-shrink-0 text-sm space-y-2">
                  {genres.length > 0 && (
                    <p className="text-white/90 leading-snug">
                      <span className="text-white/40">{t('media.genres')}: </span>
                      {genres.slice(0, 3).join(", ")}
                    </p>
                  )}
                  {movie.vote_average > 0 && (
                    <p className="text-white/90">
                      <span className="text-white/40">{t('media.rating')}: </span>
                      {movie.vote_average.toFixed(1)} / 10
                    </p>
                  )}
                  {isTvShow ? (
                    <p className="text-white/90">
                      <span className="text-white/40">{t('media.details')}: </span>TV Series
                    </p>
                  ) : (
                    <p className="text-white/90">
                      <span className="text-white/40">{t('media.details')}: </span>Movie
                    </p>
                  )}
                </div>
              </div>
