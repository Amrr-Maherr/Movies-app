...
const GenderButton = memo(function GenderButton({
  ...
  return (
    <button
      onClick={handleClick}
      className={`px-4 py-2 sm:px-5 sm:py-1.5 rounded-full text-sm font-medium transition-all duration-200 whitespace-nowrap ${
        isActive
...
const LetterButton = memo(function LetterButton({
  ...
  return (
    <button
      onClick={handleClick}
      className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all duration-200 ${
        isActive
...
const PeopleFilters = memo(
  ({
    ...
    const activeCount =
      (selectedGender !== "all" ? 1 : 0) + (selectedLetter !== "all" ? 1 : 0);
    ...
    const handleLetterClick = useCallback(
      (letter: string) => {
        handelUrl({
          lang: { code: letter === "All" ? "" : letter },
          key: "letter",
          set: onLetterSelect,
        });
      },
      [handelUrl, onLetterSelect],
    );
